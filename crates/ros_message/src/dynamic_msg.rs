...
use crate::{DataType, FieldCase, FieldInfo, MessagePath, MessageValue, Msg, Value};
...
impl DynamicMsg {
    pub fn new(message_type: &str, message_definition: &str) -> Result<Self> {
        ...
        let message_src = message_bodies.next().ok_or_else(|| {
            anyhow::anyhow!(
    ...
    pub fn dependencies(&self) -> &HashMap<MessagePath, Msg> {
        &self.dependencies
    }

    // fn flatten_msg(&self, msg: &Msg) -> Result<[Msg]> {
    //     for field in self.msg.fields() {
    //         field
    //         // match field.datatype() {
    //         //     DataType::LocalMessage(name) => {
    //         //         let path = msg.path().peer(name);
    //         //         let dependency = self.get_dependency(&path)?;

    //         //         self.flatten_msg(msg)
    //         //     }
    ...
    //     self.flatten_msg(&self.msg)?;
    //     Ok(())
    // }

    pub fn from_headers(headers: HashMap<String, String>) -> Result<Self> {
        let message_type = headers.get("type").ok_or(anyhow::anyhow!("Missing header `type`"))?;
        let message_definition = headers
            .get("message_definition")
            .ok_or(anyhow::anyhow!("Missing header `message_definition`"))?;
    ...
    fn parse_msg(message_type: &str, message_src: &str) -> Result<Msg> {
        let message_path = message_type
            .try_into()
            .map_err(|e| anyhow::anyhow!("Message type {} is invalid", message_type))?;
        Msg::new(message_path, message_src)
            .map_err(|e| anyhow::anyhow!("Failed to parse message {}", message_type))
    }
    ...
    fn parse_dependency(message_body: &str) -> Result<Msg> {
        ...
        let (message_type_line, message_src) = message_body
            .split_once('\n')
            .ok_or(anyhow::anyhow!("Message dependency is missing type declaration"))?;
        let cap = RE_DESCRIPTOR_MSG_TYPE
            .captures(message_type_line)
            .ok_or_else(|| anyhow::anyhow!("Failed to parse message type line `{}`", message_type_line))?;
    ...
    fn decode_local_message_field_array(
        &self,
        parent: &MessagePath,
        field: &FieldInfo,
        array_length: Option<usize>,
        ...
        let mut result: Vec<_> = Vec::with_capacity(array_length);
        for _ in 0..array_length {
            result.push(self.decode_message(dependency, r)?)
        }
        Ok(result.into())
    }

    fn decode_global_message_field_array(
        &self,
        parent: &MessagePath,
        field: &FieldInfo,
        array_length: Option<usize>,
    ...
    fn decode_field_array(
        ...
        Ok(match field.datatype() {
            DataType::Bool => bool::decode_vec(array_length, r)?.into(),
            DataType::I8(_) => i8::decode_vec(array_length, r)?.into(),
            DataType::I16 => i16::decode_vec(array_length, r)?.into(),
            DataType::I32 => i32::decode_vec(array_length, r)?.into(),
            DataType::I64 => i64::decode_vec(array_length, r)?.into(),
            DataType::U8(_) => u8::decode_vec(array_length, r)?.into(),
            DataType::U16 => u16::decode_vec(array_length, r)?.into(),
            DataType::U32 => u32::decode_vec(array_length, r)?.into(),
            DataType::U64 => u64::decode_vec(array_length, r)?.into(),
            DataType::F32 => f32::decode_vec(array_length, r)?.into(),
            DataType::F64 => f64::decode_vec(array_length, r)?.into(),
            DataType::String => String::decode_vec(array_length, r)?.into(),
            DataType::Time => Time::decode_vec(array_length, r)?.into(),
            DataType::Duration => Duration::decode_vec(array_length, r)?.into(),
            DataType::LocalMessage(name) => {
                self.decode_local_message_field_array(parent, field, array_length, r, name)?
            }
            DataType::GlobalMessage(path) => {
                self.decode_global_message_field_array(parent, field, array_length, r, path)?
            }
        })
