...
pub trait RosMsg: std::marker::Sized {
    ...
    #[inline]
    fn decode_vec<R: io::Read>(array_length: Option<usize>, mut r: R) -> io::Result<Vec<Self>> {
        let array_length = match array_length {
            Some(v) => v,
            None => u32::decode(r.by_ref())? as usize,
        };

        let mut result: Vec<Self> = Vec::with_capacity(array_length);
        for _ in 0..array_length {
            result.push(Self::decode(&mut r)?);
        }
        Ok(result)
    }
}
